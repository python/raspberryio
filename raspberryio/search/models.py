...
def load_search_model_indexes():
    ...
    a side-effect.
    """
    index = settings.SEARCH_MODEL_INDEXES.items()
    model_fields = clean_index(index)
    model_fields = clean_model_fields(model_fields)
    model_classes = []
    for ModelKls, fields in model_fields:
        make_search_proxy_model(ModelKls, fields)
        model_classes.append(ModelKls)
...
def clean_index(index):
    ...
    for appname_model, fields in index:
        try:
            ...
            err_msg = 'SEARCH_MODEL_INDEXES setting key {0} is in the wrong ' + \
            'format'.format(appname_model)
            logging.error(err_msg)
            continue
        ModelKls = models.get_model(appname, modelname)
        if ModelKls is None:
            err_msg = 'SEARCH_MODEL_INDEXES key {0} refers to a model that ' + \
            'cannot be found.'.format(appname_model)
            logging.error(err_msg)
            continue
...
# On import, create search proxy models from the SEARCH_MODEL_CHOICES
